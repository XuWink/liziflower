  ...
  <el-dialog v-model="dialogTableVisible" title="订单详情">
    <el-table fit :data="gridData">
      ...
      <el-table-column align="center" label="评价">
        <template #default="scope">
          <el-button @click="Remark(scope.$index,scope.row)" type="primary" plain>评价</el-button>
        </template>
      </el-table-column>
    </el-table>
  </el-dialog>
...
export default {
  ...
  setup() {
    ...
    const getAllFinishedOrders = async () => {//获取所有订单
      await request.get(apiList.getAllFinishedOrders, {
        params: {
    ...
    const Remark = (index: number, row: any) => {//对单一商品进行评论
      dialogTableVisible.value = false;//关闭对话框
      // console.log("row.name"+row.name)
      router.push({path: '/editor', query: {flowerid: row.flowerid,flowername:row.name, flowerimage: row.flowerimage}})
      console.log(index);
      // console.log(row);
